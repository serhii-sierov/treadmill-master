...
export async function applySegmentToTreadmill(segment: Segment): Promise<void> {
  await getTreadmillAdapter().applySegmentTargets(segment.speedKmh, segment.inclinePercent);
}
...
export async function startSegmentOnTreadmill(
  ...
  await getTreadmillAdapter().startSegment(segment.speedKmh, segment.inclinePercent, options);
}
